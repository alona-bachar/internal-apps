#!/usr/bin/env node
// Deploy il-pod-slack-poster-v1 to the cto-office tenant.
// PUT silently drops `code` on Wonderful Functions, so this script DELETE+POSTs.
//
// Usage:
//   WONDERFUL_API_KEY="<key>" node functions/il-pod-slack-poster/deploy-function.mjs
...
const SRC = path.join(__dirname, "il-pod-slack-poster.ts");
...
const SLUG = process.env.FUNCTION_SLUG ?? "il-pod-slack-poster-v1";
const NAME = process.env.FUNCTION_NAME ?? "IL Pod Slack Poster v1";
