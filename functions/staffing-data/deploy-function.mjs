#!/usr/bin/env node
// Deploy pod-staffing-data-v109 to the cto-office tenant.
// PUT silently drops `code` on Wonderful Functions, so this script DELETE+POSTs.
//
// Usage:
//   WONDERFUL_API_KEY="<key>" node functions/staffing-data/deploy-function.mjs
...
const SRC = path.join(__dirname, "pod-staffing-data.ts");
const ESBUILD = path.resolve(
  __dirname,
  process.env.ESBUILD_BIN ?? "../../apps/staffing/node_modules/.pnpm/esbuild@0.21.5/node_modules/esbuild/bin/esbuild",
);
...
const SLUG = process.env.FUNCTION_SLUG ?? "pod-staffing-data-v109";
const NAME = process.env.FUNCTION_NAME ?? "Pod Staffing Data v109";
