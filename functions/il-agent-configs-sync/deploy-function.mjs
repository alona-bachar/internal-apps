#!/usr/bin/env node
// Deploy il-agent-configs-sync-v1 to the cto-office tenant.
...
const SRC = path.join(__dirname, "il-agent-configs-sync.ts");
const ESBUILD = [
...
const SLUG = process.env.FUNCTION_SLUG ?? "il-agent-configs-sync-v1";
const NAME = process.env.FUNCTION_NAME ?? "IL Agent Configs Sync v1";
